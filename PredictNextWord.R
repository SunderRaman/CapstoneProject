if (!exists("n5")) {
  n5 <- read.csv("n5.csv", stringsAsFactors=FALSE, fileEncoding="latin1")
}
if (!exists("n4")) {
  n4 <- read.csv("n4.csv", stringsAsFactors=FALSE, fileEncoding="latin1")
}
if (!exists("n3")) {
  n3 <- read.csv("n3.csv", stringsAsFactors=FALSE, fileEncoding="latin1")
}
if (!exists("n2")) {
  n2 <- read.csv("n2.csv", stringsAsFactors=FALSE, fileEncoding="latin1")
}
...
## Implementbackoff algorithm
findWord <- function(x, alpha=0.4, getNrows=20, showNresults=1) {
  nextword <- ""
  #if (x == "") {
  #  return("the")
  #}
...
phrase <- "You are the reason why I smile everyday. Can you follow me please? It would mean the "
